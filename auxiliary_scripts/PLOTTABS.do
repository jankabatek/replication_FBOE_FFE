...
*			pattern2 = visual separation of distinct graph lines + colours
...
*------------------j.kabatek@unimelb.edu.au, 06/2021, (c)----------------------*
...
program define PLOTTABS
	syntax varlist(max=2) [if], [clear DIVide(real 1) dif frame(string) FMT(string) GRaph(name) GRAYscale ///
								 IFLabel NOGen NODraw OPTions(string) PATtern pattern2 PLOTonly  ///  
								 RELative row  Title(string) TWOff yzero ] 		
	...
	qui {
		...
		** pattern definition for visual separation of distinct graph lines
		if "`pattern'" !="" { 
		 local pattern1  msymbol(O) lpattern(solid)  	 
		 local pattern2  msymbol(D) lpattern(shortdash)  			 
		 local pattern3  msymbol(S) lpattern(longdash)   		  	 
		 local pattern4  msymbol(T) lpattern(longdash_shortdash)   
		 local pattern5  msymbol(T) lpattern(solid) 
		} 
		...
		** OUTPUT (sourced from the respective frame): 
		if "`nogen'" == ""  frame `frame_pt':  `graph_syntax'  ,  ysize(5) title(`title') graphregion(fcolor(white) lcolor(white)) `options' `format' `nodraw'
